...
export const MetricPillCard: React.FC<MetricPillCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative flex items-center gap-3 p-3 rounded-2xl border transition-all duration-200 select-none shadow-sm ${
        onClick ? 'cursor-pointer active:scale-95' : ''
      } ${styles.container} ${className}`}
    >
      {/* Icon Badge */}
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 shadow-inner ${styles.iconBg}`}
      >
        {icon}
      </div>
      ...
      <div className="flex-1 min-w-0 pr-1">
        <div className={`text-[11px] font-medium truncate ${styles.label}`}>
          {label}
        </div>
        <div className={`text-sm sm:text-base font-bold tracking-tight truncate ${styles.value}`}>
          {value}
          {subtitle && (
            <span className="text-[11px] font-normal text-slate-300/70 ml-1">
              {subtitle}
            </span>
          )}
        </div>
