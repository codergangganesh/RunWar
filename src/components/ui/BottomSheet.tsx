...
export const BottomSheet: React.FC<BottomSheetProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center select-none">
      ...
      <div
        onClick={onClose}
        className={`absolute inset-0 bg-emerald-950/40 dark:bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 ease-out ${
      ...
      <div
        ...
        className={`relative z-10 w-full max-w-md bg-white dark:bg-slate-900 border-t border-x border-emerald-100 dark:border-slate-800 rounded-t-[32px] shadow-[0_-12px_40px_rgba(16,185,129,0.15)] dark:shadow-[0_-12px_40px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[85vh] will-change-transform ${
      ...
      >
        ...
        {/* Sheet Header */}
        {title && (
          <div className="flex items-center justify-between px-6 py-3 border-b border-emerald-100 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              {icon && <span className="text-emerald-600 dark:text-emerald-400">{icon}</span>}
              <h3 className="text-base font-black text-emerald-950 dark:text-white tracking-tight">{title}</h3>
            </div>
            <button
              onClick={onClose}
